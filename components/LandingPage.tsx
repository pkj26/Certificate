...
interface LandingPageProps {
  onStart: (mode?: 'course' | 'salary' | 'experience' | 'resume') => void;
...
const LandingPage: React.FC<LandingPageProps> = ({ onStart, onNavigate }) => {
  ...
  const tools = [
    { key: 'resume', name: 'AI Resume Builder' },
    { key: 'course', name: 'Certificate Maker' },
    { key: 'experience', name: 'Experience Letter' },
    { key: 'salary', name: 'Salary Slip Generator' },
  ];
    ...
    <div className="min-h-screen bg-white font-sans text-gray-900 overflow-x-hidden">
      ...
      <nav className="flex items-center justify-between px-6 lg:px-12 py-6 bg-white/90 backdrop-blur-md sticky top-0 z-50 border-b border-gray-100 shadow-sm">
        ...
        <div className="flex items-center space-x-6">
           ...
           <button onClick={() => onStart('resume')} className="bg-blue-900 text-white px-6 py-2.5 rounded-full font-bold hover:bg-blue-800 transition shadow-lg hover:shadow-blue-900/30 transform hover:-translate-y-0.5">
             Create Resume
           </button>
        </div>
      </nav>
      ...
      <header className="relative px-6 lg:px-12 py-20 lg:py-24 text-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-5xl mx-auto">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-12">
            <button onClick={() => onStart('resume')} className="w-full sm:w-auto text-lg bg-blue-600 text-white px-10 py-5 rounded-full font-bold shadow-xl shadow-blue-600/30 hover:shadow-2xl hover:bg-blue-700 hover:-translate-y-1 transition-all">
      ...
      {/* Statistics Strip - Adds Authority */}
      ...
      <section id="downloads" className="px-6 lg:px-12 py-16 bg-white">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {/* SEO Card 0: AI Resume Builder */}
            <div className="lg:col-span-2 bg-blue-50 rounded-2xl p-8 border-2 border-blue-500 hover:shadow-2xl transition-all cursor-pointer group flex flex-col items-center text-center" onClick={() => onStart('resume')}>
      ...
      {/* HEAVY CONTENT SECTION FOR SEO (Fixes "Empty Site" issue) */}
      ...
      <section id="features" className="px-6 lg:px-12 py-24 bg-white">
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
           <FeatureCard 
             icon="🤖" 
             title="AI Auto-Fill" 
             desc="Select your job title and get a complete, high-quality resume draft instantly. Save hours of writing." 
           />
           <FeatureCard 
             icon="📄" 
             title="Multi-Page Options" 
             desc="Choose the perfect length for your career. Generate anything from a 1-page summary to a 5-page detailed CV." 
           />
           <FeatureCard 
             icon="✅" 
             title="ATS-Friendly Templates" 
             desc="Our templates are designed to be easily parsed by Applicant Tracking Systems, increasing your chances of getting noticed." 
           />
            <FeatureCard 
             icon="🎨" 
             title="10+ Certificate Designs" 
             desc="Choose from Classic Gold, Modern Blue, and more for professional computer course certificates." 
           />
           <FeatureCard 
             icon="🔒" 
             title="QR Code Verification" 
             desc="Each certificate comes with a unique, scannable QR code that links to a secure verification page." 
           />
           <FeatureCard 
             icon="🖨️" 
             title="Instant PDF Download" 
             desc="Download high-resolution PDFs of your resumes and certificates, ready for printing or digital submission." 
           />
        </div>
      </section>
         ...
         <div className="max-w-4xl mx-auto">
            ...
            <div className="grid gap-6">
               <FaqItem 
                 ...
                 answer="Yes, 100%! Our AI Resume Builder, along with all our document generators, is completely free to use. The platform is supported by ads." 
               />
               <FaqItem 
                 question="How many pages should my resume be?" 
                 answer="For freshers and professionals with under 10 years of experience, a 1-page resume is ideal. For those with more extensive experience, a 2-page resume is acceptable. Our tool provides options from 1 to 5 pages to suit every need." 
               />
               <FaqItem 
                 question="What is an ATS-friendly resume?" 
                 answer="An ATS-friendly resume uses a clean, simple format with standard fonts and clear headings that can be easily read by the software recruiters use to screen candidates. All our templates are ATS-optimized." 
               />
               <FaqItem 
                 question="Can I download my resume in Word format?" 
                 answer="Currently, we only support high-quality PDF downloads as it preserves formatting across all devices. We may add other formats in the future." 
               />
            </div>
         </div>
      </section>
         ...
         <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-4 gap-12 mb-16">
            ...
            <div>
               ...
               <ul className="space-y-3 text-blue-300 text-sm">
                 <li><button onClick={() => onStart('resume')} className="hover:text-white transition text-left">AI Resume Builder</button></li>
                 <li><button onClick={() => onStart('course')} className="hover:text-white transition text-left">Certificate Generator</button></li>
                 <li><button onClick={() => onStart('experience')} className="hover:text-white transition text-left">Experience Letters</button></li>
                 <li><button onClick={() => onStart('salary')} className="hover:text-white transition text-left">Salary Slips</button></li>
               </ul>
            </div>
            <div>
               <h4 className="font-black text-white uppercase tracking-widest mb-6">Legal</h4>
               <ul className="space-y-3 text-blue-300 text-sm">
                 <li onClick={() => onNavigate?.('privacy')} className="hover:text-white transition cursor-pointer">Privacy Policy</li>
                 <li onClick={() => onNavigate?.('terms')} className="hover:text-white transition cursor-pointer">Terms of Service</li>
                 <li onClick={() => onNavigate?.('about')} className="hover:text-white transition cursor-pointer">About Us</li>
               </ul>
            </div>
         </div>
